...
def _analyze_task_type(request: ChatRequest) -> dict:
    """Analyze the task to determine optimal provider."""
    model = request.model.lower().strip()
    user_text = _last_user_message(request)
    
    # Explicit model preference overrides
    if model.startswith("gemini"):
        return {"preferred": "gemini", "reason": "user_requested_gemini"}
    if model.startswith("gpt") or model.startswith("openai"):
        return {"preferred": "openai", "reason": "user_requested_openai"}
    ...
    if openai_match > 0:
        return {"preferred": "openai", "reason": "openai_specialty"}
    
    # Default: cost-optimized (Gemini cheaper)
